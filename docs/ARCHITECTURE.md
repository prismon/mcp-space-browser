...
└──────────┘     └──────────────────┬───────────────────────────┘
          ...
          ┌──────────────────────────────────────────────┐
          │              Database (SQLite)                │
          │   entries, attributes, resource_sets, plans   │
          └──────────────────┬───────────────────────────┘
                             │
                             ▼
          ┌──────────────────────────────────────────────┐
...
1. **Scan**: `scan` tool invokes crawler on filesystem paths → entries + attributes stored in SQLite
...
| `pkg/database` | SQLite abstraction: entries, attributes, resource sets, plans, sources, rules, jobs |
...
| `internal/models` | Shared data structures: Entry, Attribute, ResourceSet, Plan, Source, Rule |
...
-- Extensible key-value attributes (multiple per entry)
attributes (entry_path + key PK, value, source, computed_at)
...
- **Attribute System**: Extensible key-value pairs per entry, queryable via the `where` clause
