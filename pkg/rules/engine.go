...
func (e *Engine) ensureResourceSet(name string) (int64, error) {
	...
	result, err := e.db.Exec(`
		INSERT INTO resource_sets (name, description, created_at, updated_at)
		VALUES (?, ?, ?, ?)
	`, name, "Auto-created by rule execution", now, now)
	if err != nil {
		return 0, err
	}
...
func (e *Engine) createExecution(ruleID, resourceSetID int64) (int64, error) {
	result, err := e.db.Exec(`
		INSERT INTO rule_executions (rule_id, resource_set_id, entries_matched, entries_processed, status)
		VALUES (?, ?, 1, 0, 'running')
...
func (e *Engine) addToResourceSet(setID int64, path string) error {
	now := time.Now().Unix()
	_, err := e.db.Exec(`
		INSERT OR IGNORE INTO resource_set_entries (set_id, entry_path, added_at)
		VALUES (?, ?, ?)
	`, setID, path, now)
	return err
}
